...
const ExpandableProjectCard: React.FC<ExpandableProjectCardProps> = ({ project, index }) => {
  ...
  return (
    <motion.div
      ...
      className={`group relative rounded-xl overflow-hidden cursor-pointer ${project.featured ? 'md:col-span-2 lg:col-span-1' : ''
        }`}
    >
      <SpotlightCard className="bg-darker border border-secondary/20 h-full p-0">
        {/* Project Image Area with TiltedCard */}
        <div className="relative h-48 w-full overflow-hidden">
          ...
          {project.featured && (
            <div className="absolute top-4 left-4 px-3 py-1 bg-primary/20 backdrop-blur-sm rounded-full text-primary text-sm font-medium border border-primary/30 z-20">
              Featured
            </div>
          )}
          ...
          <AnimatePresence>
            {isHovered && !isExpanded && (
              <motion.div
                ...
                animate="visible"
                exit="hidden"
                className="absolute inset-0 flex items-center justify-center bg-black/60 backdrop-blur-sm z-10 pointer-events-none"
              >
          ...
          {isExpanded && (
            <div className="absolute bottom-4 right-4 px-3 py-1 bg-black/50 backdrop-blur-sm rounded-lg text-white text-xs font-medium z-20">
              Click to collapse
            </div>
          )}
        </div>
                ...
                <div className="pt-4 border-t border-secondary/20">
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                      ...
                      <span className={`inline-block px-2 py-1 text-xs rounded-full ${project.status === 'completed'
